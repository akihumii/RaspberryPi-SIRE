...
static void waitForSylph(void){
    ...
    bool isBind = false;
    while(!isBind){
        isBind = bind(socket_desc, (struct sockaddr *)&server, sizeof(server));
...
void verifyCounter(){
    if (isFrDetected == false){
        countIndex = findSyncPulse(parallelbuffer, BUFFER_SIZE) -1;
        if (countIndex != -1){
            isFrDetected = true;
            counter = parallelbuffer[countIndex];
            printf("new count: %d %d", counter, parallelbuffer[countIndex]);
        }
    }

    while(countIndex < BUFFER_SIZE && isFrDetected == true){
        if (counter != parallelbuffer[countIndex]){
            if (errCount<5){
                printf("err: %d %d", counter, parallelbuffer[countIndex]);
                counter = parallelbuffer[countIndex];
            }
            else {
                isFrDetected = false;
